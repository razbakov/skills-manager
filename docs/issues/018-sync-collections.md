...
- After every collection file change, the app stages, commits, and pushes the affected file(s) automatically, similar to how adopted skills are committed today.
...
- If the git commit or push fails, the app shows a notification with the reason but does not block the user from continuing.
