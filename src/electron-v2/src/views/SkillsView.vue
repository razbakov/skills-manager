...
<template>
  <div class="flex h-full flex-col">
    <div class="border-b px-3 py-2">
      <div class="inline-flex rounded-lg border bg-muted/30 p-1">
        <button
          ...
          :class="
            store.skillsViewMode.value === 'list'
              ? 'bg-background text-foreground shadow-sm'
              : 'text-muted-foreground hover:text-foreground'
          "
          @click="store.skillsViewMode.value = 'list'"
        >
          Skills
        </button>
        <button
          class="rounded-md px-3 py-1.5 text-xs font-medium transition-colors cursor-pointer"
          :class="
            store.skillsViewMode.value === 'groups'
              ? 'bg-background text-foreground shadow-sm'
              : 'text-muted-foreground hover:text-foreground'
          "
          @click="store.skillsViewMode.value = 'groups'"
        >
          Collections
        </button>
      </div>
    ...
    <InstalledView v-if="store.skillsViewMode.value === 'groups'" class="min-h-0 flex-1" />
